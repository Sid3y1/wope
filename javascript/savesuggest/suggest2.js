  ...
  */      break;
  ...
  this.list_mailbox_remote = function(mbox, page, add_url)
    ...
    this.http_request('list', url+add_url);
					//	parent.frameSize((parseInt(document.getElementById("messagelist").offsetHeight) + 140) +'px','javascript2');
    };
  ...
  this.ksearch_get_results = function()
    ...
    if (c && a_results.length)
      ...
      for (i=0; i<a_results.length; i++)
        ...
        li.innerHTML = '<span id="span'+a_result_ids[i]+'" onmousedown="rcmail.thisone('+a_result_ids[i]+')" onmouseover="rcmail.reselect('+a_result_ids[i]+');">'+a_results[i].replace(/</, '&lt;').replace(/>/, '&gt;')+'</span>';
