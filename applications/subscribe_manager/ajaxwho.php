...
if(isset($_GET['unlogin'])){
	if($db->result($db->query("SELECT count(*) FROM subscribeManager_gens WHERE login='".$_GET['unlogin']."' AND id_event = '".$_GET['id_event']."'"),0)==0){
		$page->msgError("Erreur, non pr�sent dans la base de donn�e !");
	}else{
		$db->query("DELETE FROM subscribeManager_gens WHERE login='".$_GET['unlogin']."' AND id_event = '".$_GET['id_event']."'");
		$page->msgInfo($_GET['unlogin'].' desinscrit avec succes !');
	}
...
if(isset($_GET['verif'])){
	...
	if($db->numRows($res)==0){
		exit('<h1 style="background:red;font-size:30;">NO CORESPONDance<br /> verifiez sur la liste </h1>');
	}
	$lui = $db->fetchArray($res);
	$oui = $db->result($db->query("SELECT count(*) FROM subscribeManager_gens WHERE login='".$lui['login']."' AND id_event='".$_GET['id_event']."' ",1),0);
	$deja = $db->result($db->query("SELECT count(*) FROM  subscribeManager_gens WHERE isin='Y' AND login ='".$lui['login']."'  AND id_event='".$_GET['id_event']."'  "),0);
	if($deja != 0){
		$page->msgError("Le mec il a deja pass� sa carte");
		echo '<div style="background:#cccccc;">';
	}

 echo '<h1>'.$db->result( $db->query("SELECT count(*) FROM subscribeManager_gens WHERE isin='N'  AND id_event='".$_GET['id_event']."'"),0).'</h1>';
	$db->query("UPDATE subscribeManager_gens SET isin='Y' where login='".$lui['login']."'  AND id_event='".$_GET['id_event']."'    ");
...
if($_GET['code']=='cores'){
	if(1==$db->result($db->query("SELECT count(login) FROM usersInfo WHERE login = '".trim($_GET['login'])."'"),0)){
		$db->query("UPDATE usersInfo SET codebarre = '".$_GET['codebarre']."' , datecodebarre=now() WHERE login='".$_GET['login']."'",1);
		echo '<h2>CORESPONDANCE FAITE !!</h2>';
	}else{
		echo '<h2>CORESPONDANCE PAS FAITE !!</h2>';
	}
	// focus
	echo '<script>document.getElementById(\'codebarre\').focus();</script>';
	exit();
...
if($db->numRows($res)==0){
	...
	?>
		...
		<input type="button" value="Creer la corespondance" onclick="ajaxId('ajaxwho.php?id_event=<?=$id_event?>&code=cores&codebarre='+document.getElementById('coresCode').value+'&login='+document.getElementById('coresLogin').value,'who');return false;" />
